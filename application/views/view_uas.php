...
<div class="container-fluid">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle">Tambah Data Ekspedisi</h5>
                ...
                <div class="modal-body">
                    <form action="" method="POST" id="dataForm">
                        ...
                        <div class="form-group">
                            <label>Telfon</label>
                            <input type="text" id="telfon" name="telfon" class="form-control">
                        </div>
    ...
    function showData(id) {
        FormEdit();
        $.ajax({
            url: '<?= base_url("uas/showData/"); ?>' + id,
            dataType: 'JSON',
            success: function(result) {
                $('#pengirim').val(result.pengirim);
                $('#penerima').val(result.penerima);
                $('#alamat').val(result.alamat);
                $('#telfon').val(result.telfon);
    ...
    function insertData() {
        if ($('#pengirim').val() == '' && $('#penerima').val() == '' && $('#alamat').val() == '' && $('#telfon').val() == '' && $('#berat') ) {
            Swal.fire('Ooppss!!', 'Mohon mengisi nama pengirim!', 'warning');
        }
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.ping == 200) {
                        keTable();
                        loadTable();
                        toastr.success('Data ekspedisi telah ditambahkan!', 'Created!!', {
    ...
    if ($('#pengirim').val() == '' && $('#penerima').val() == '' && $('#alamat').val() == '' && $('#telfon').val() == '' && $('#berat')) {
            Swal.fire('Ooppss!!', 'Mohon mengisi nama ekspedisi!', 'warning');
        }
         else {
            $.ajax({
                url: '<?= base_url("uas/updateData/"); ?>' + id,
                type: 'POST',
                dataType: 'JSON',
                data: $('#dataForm').serialize(),
                success: function(result) {
                    if (result.ping == 200) {
                        keTable();
                        loadTable();
                        toastr.success('Data ekspedisi telah diupdate!', 'Updated!!', {
                            showMethod: 'slideDown',
                            hideMethod: 'slideUp',
                            timeOut: 2000
                        });
    ...
    function deleted(id) {
        Swal.fire({
            ...
            text: 'Penghapusan data ekspedisi!',
            type: 'warning',
